(() => {
    ...
    function sendLatexToRenderWindow(rawLatex) {
        const wrappedLatex = wrapLatex(rawLatex);
        if (renderWindow && !renderWindow.closed && wrappedLatex !== lastLatexText) {
            lastLatexText = wrappedLatex;
            renderWindow.postMessage({ type: 'update', text: wrappedLatex }, '*');
        }
    }
    ...
    function init() {
        ...
        bindInputs();
        bindFocusWatcher();
        console.log('✅ LaTeX 实时渲染器已启用，内容已自动加上 $ 包裹');
    }
